...
import { defaultProfile,defaultUserJourney,PROFILE_KEY,USER_JOURNEY_KEY,type OnboardingStep,type UserJourney,type WorldAccessMode } from './stores/profileStore';
...
export default function App(){
  ...
  const profile={...defaultProfile,...storedProfile,usagePurposes:storedProfile.usagePurposes??[],preferredPlaceCategories:storedProfile.preferredPlaceCategories??[]};
  ...
  const continueOnboarding=()=>setPage(journey.onboardingStep==='terms'?'terms':journey.onboardingStep==='verification'?'verify':'create');
  ...
  const finishVerification=(mode:Exclude<WorldAccessMode,'unverified'>)=>{
    if(membershipComplete){setJourney({...journey,authenticated:true,membershipComplete:true,accessMode:mode});setPage('game')}
    else{setJourney({...journey,authenticated:true,accessMode:mode,onboardingStep:'profile'});setPage('create')}
  };
  ...
  if(page==='landing'){
    const actionLabel=journey.authenticated&&membershipComplete?'체험하기':onboardingInProgress?'가입 이어서 하기':'멀티버스 입장하기';
    return <LandingPage onStart={startFromLanding} onUserClick={()=>setPage('account')} actionLabel={actionLabel} userName={journey.authenticated&&membershipComplete?profile.nickname:undefined}/>;
  }
  ...
  if(page==='create')return <CreateProfilePage initial={profile} initialStep={journey.onboardingStep==='character'?2:1} onProgress={saveProgress} onComplete={finishSignup}/>;
  if(page==='complete')return <SignupCompletePage profile={profile} onEnter={()=>setPage('game')}/>;
  if(page==='account')return <CreateProfilePage initial={profile} editMode onCancel={()=>setPage('landing')} onLogout={()=>{setJourney({...journey,authenticated:false});setPage('landing')}} onComplete={updated=>{setProfile(updated);setPage('landing')}}/>;
