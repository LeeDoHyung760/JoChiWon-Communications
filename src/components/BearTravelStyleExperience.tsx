...
export function BearTravelStyleExperience({userKey,mapId}:{userKey:string;mapId:MapId}){
  ...
  return <div className="bear-style-ui">
    <aside className="bear-style-status"><Compass size={16}/><div><small>베어트리 AI 탐험 연구소</small><b>{result?'나의 탐험 프로필 완성':readyForFinal?'모든 장소 관찰 완료':'여행 스타일 분석 중'} · 탐험 장소 {Object.keys(progress.choices).length}/3</b></div>{result?<button type="button" onClick={()=>setResultOpen(true)}>프로필 보기</button>:readyForFinal?<button type="button" onClick={()=>setCompanionOpen(true)}>최종 분석하기</button>:<span>{progress.photoCaptured?'포토 기록 완료':'자유롭게 둘러보세요'}</span>}</aside>
    {introOpen&&<section className="bear-style-overlay"><div className="bear-style-modal bear-style-intro"><i>🐻</i><small>자연을 탐험하며 나의 여행 방식을 발견하는 곳</small><h2>베어트리 AI 탐험 연구소</h2><p>폭포, 동굴, 큰 나무를 자유롭게 둘러보세요. 관찰 방식과 이동 기록을 바탕으로 나의 여행 스타일을 분석합니다.</p><button className="bear-style-done" onClick={()=>setIntroOpen(false)}>자유 탐험 시작하기</button></div></section>}
