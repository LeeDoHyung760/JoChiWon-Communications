...
const detailTabs:Array<{id:DetailTab;label:string}>=[{id:'overview',label:'프로젝트 설명'},{id:'members',label:'참여자 목록'},{id:'chat',label:'채팅'},{id:'memo',label:'공유 메모'},{id:'places',label:'추천 장소'},{id:'schedule',label:'AI 추천 일정'}];
...
export function CampusProjectRoom({onGovernment,onNotice}:{onGovernment:()=>void;onNotice:(message:string)=>void}){
  ...
  if(selected)return <section className="campus-project-detail">
    <header><button type="button" onClick={()=>setSelected(null)}><X size={14}/> 닫기</button><span>{selected.emoji}</span><div><small>COLLABORATION PROJECT</small><h2>{selected.title}</h2><p>{selected.goal} · {selected.members.length}/{selected.capacity}명 참여</p></div></header>
    ...
    <div className="campus-project-detail-body">
      {detailTab==='overview'&&<article className="project-overview"><small>PROJECT PURPOSE</small><h3>{selected.goal}</h3><p>{selected.description}</p><div>{selected.tags.map(tag=><span key={tag}>#{tag}</span>)}</div><button type="button" disabled={selected.members.includes('나')||selected.members.length>=selected.capacity} onClick={()=>join(selected)}>{selected.members.includes('나')?'참여 중':selected.members.length>=selected.capacity?'모집 완료':'프로젝트 참여하기'}</button></article>}
      ...
      {detailTab==='schedule'&&<article className="project-ai-schedule"><header><Sparkles size={18}/><div><small>AI PROJECT PLANNER</small><b>참여자 취향을 반영한 추천 일정</b></div></header>{[['14:00','학생회관에서 만나기'],['14:30',selected.goal],['16:30','사진과 기록 함께 정리'],['17:00','카페에서 다음 활동 정하기']].map(([time,activity])=><div key={time}><time>{time}</time><span>{activity}</span></div>)}<button type="button" onClick={onGovernment}>정부청사에서 최종 코스 만들기 <ArrowRight size={14}/></button></article>}
    </div>
  </section>;
  return <>
    <div className="campus-section-title"><div><small>③ 프로젝트실 · GOAL-BASED COLLABORATION</small><h2>함께 세종에서 무엇을 할지 계획해요</h2><p>단순한 채팅을 넘어 목표, 참여자, 장소와 일정을 하나의 프로젝트로 완성합니다.</p></div><button type="button" className="campus-create-button" onClick={()=>setComposer(value=>!value)}><Plus size={15}/> 프로젝트 생성</button></div>
    ...
    <section className="campus-project-recommend"><Sparkles size={20}/><div><small>AI 프로젝트 추천</small><b>“수목원 사진 프로젝트가 잘 맞을 것 같습니다.”</b><p>자연 선호 · 사진 촬영 · 여유형 여행 스타일을 바탕으로 추천했어요.</p></div><button type="button" onClick={()=>setSelected(projects.find(project=>project.id==='garden-photo')??projects[0])}>추천 보기</button></section>
    ...
    <section className="campus-project-next"><Calendar size={18}/><div><b>프로젝트가 준비되면 정부청사로 이어져요</b><p>AI가 참여자의 취향과 프로젝트 목표를 실제 세종 방문 코스로 정리합니다.</p></div><button type="button" onClick={onGovernment}>정부청사 이동</button></section>
  </>;
}
