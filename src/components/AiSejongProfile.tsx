...
import { BarChart3, BookOpen, Camera, ChevronDown, ChevronRight, Edit3, Flower2, Heart, Leaf, LockKeyhole, Music2, Sparkles, Trophy, X } from 'lucide-react';
...
export function AiSejongProfile({ profile, onClose, onEdit }: { profile: UserProfile; onClose: () => void; onEdit: () => void }) {
  ...
  const keywords = [
    { icon: <Leaf />, label: '자연을 사랑해요', value: s.nature, tone: 'green' },
    { icon: <Music2 />, label: '문화와 공연', value: s.culture, tone: 'purple' },
    { icon: <Heart />, label: '사람과 교류', value: s.relation, tone: 'pink' },
    { icon: <Camera />, label: '순간을 기록해요', value: s.record, tone: 'orange' },
  ];
  const visibleRecords = progress.records.slice(0, showAllRecords ? 20 : 4);
  const level = Math.max(1, Math.floor(progress.points / 100) + 1);
  const courses = ai.recommendedCourse.length ? courseSeed.map((course, index) => ({ ...course, title: ai.recommendedCourse[index] || course.title })) : courseSeed;
  ...
  return <div className="ai-sejong-profile-overlay" role="dialog" aria-modal="true" aria-labelledby="ai-sejong-profile-title">
    <section className="ai-sejong-profile">
      ...
      <header className="profile-hero">
        ...
        <button className="profile-summary" onClick={() => setDetail('zones')}><div><Sparkles /><span>탐험한 공간</span><b>{progress.visitedZoneCount} / {PROFILE_ZONES.length}</b></div><div><Camera /><span>기록한 순간</span><b>{progress.records.length}</b></div><div><Trophy /><span>성장 포인트</span><b>{progress.points.toLocaleString()} P</b></div></button>
      </header>

      <div className="profile-main-grid"><div className="profile-left-column">
        <section className="profile-card keyword-card"><div className="section-title"><h3><Sparkles /> 나를 나타내는 키워드</h3><button onClick={() => setDetail('keywords')}>자세히 보기 <ChevronRight /></button></div><div className="keyword-list">{keywords.map(item => <div className={`keyword ${item.tone}`} key={item.label}><span>{item.icon}</span><b>{item.label}</b><small>{item.value}%</small></div>)}</div></section>
        <section className="profile-card radar-card"><div className="section-title"><h3><Flower2 /> 관심사 레이더</h3></div><div className="radar-content"><div className="radar-chart"><i className="axis a1"/><i className="axis a2"/><i className="axis a3"/><div className="radar-shape" style={{ opacity: Math.max(.2, progress.completion / 100) }}/><span className="r1">자연·힐링 <b>{s.nature}</b></span><span className="r2">문화·예술 <b>{s.culture}</b></span><span className="r3">탐험·발견 <b>{s.explore}</b></span><span className="r4">사진·기록 <b>{s.record}</b></span><span className="r5">사람·교류 <b>{s.relation}</b></span></div><div className="radar-copy"><b>맵에서 발견한 나의 성향</b>{progress.records.length ? <><p><Leaf /> 자연 체험 {progress.greenhouse.collected.length}개가 반영됐어요</p><p><Music2 /> 방문한 맵 {progress.visits.length}곳이 분석됐어요</p><p><Heart /> 교류 공간 {progress.campus.length}곳을 경험했어요</p></> : <p><LockKeyhole /> 맵을 돌아다니면 성향 분석이 시작돼요.</p>}</div></div></section>
      </div>
      <section className="profile-card activity-card"><div className="section-title"><h3><BookOpen /> 최근 탐험 기록</h3><button onClick={() => setDetail('records')}>전체 보기 <ChevronRight /></button></div><div className="activity-list">{visibleRecords.length ? visibleRecords.map(item => <article key={item.id}><img src={item.image} alt=""/><div><small>{item.zone}</small><b>{item.title}</b><p>{item.note}</p></div><aside><time>{formatTime(item.at)}</time><strong>+{item.point}P</strong></aside></article>) : <div className="profile-empty"><BookOpen /><b>아직 탐험 기록이 없어요</b><p>맵의 포털을 지나 새로운 장소를 방문해 보세요.</p></div>}</div>{progress.records.length > 4 && <button className="more-records" onClick={() => setShowAllRecords(value => !value)}>{showAllRecords ? '최근 기록만 보기' : '모든 기록 더보기'} <ChevronDown /></button>}</section></div>

      <section className="ai-sejong-analysis"><Sparkles /><div><small>AI 종합 분석</small><p>{progress.experienceCount ? ai.oneLineAnalysis : '세종 곳곳의 체험을 시작하면 나만의 여행 성향이 이곳에 자라납니다.'}</p></div><div className="analysis-tags"><span>{s.nature >= s.culture ? '자연 & 힐링 중심' : '문화 & 예술 중심'}</span><span>{s.explore > 55 ? '적극적 탐험가' : '차분한 탐험가'}</span><span>{s.relation > 50 ? '교류 경험 풍부' : '나만의 기록 중'}</span></div><div className="analysis-character"><CharacterPreview parts={profile.character} small /></div></section>

      <div className="profile-bottom-grid"><section className="profile-card course-card"><div className="section-title"><div><h3>당신에게 추천하는 세종 코스</h3><small>{progress.completion < 35 ? '체험 3개 이상부터 추천이 정교해져요' : '지금까지의 탐험 성향으로 추천했어요'}</small></div></div><div className="course-list">{courses.map(course => <article key={course.title}><div><img src={course.image} alt=""/>{course.badge && <span>{course.badge}</span>}</div><b>{course.title}</b><p>{course.desc}</p><small>{course.meta}</small></article>)}</div></section>
        <section className="profile-card growth-card"><div className="section-title"><div><h3>프로필 성장 히스토리</h3><small>탐험할수록 높아져요!</small></div><span className="today">오늘<br/><b>{progress.completion}% 달성!</b></span></div><div className="growth-chart"><span>100%</span><span>50%</span><span>0%</span>{growthBars(progress.completion, progress.visitedZoneCount).map((value, index) => <i key={index} style={{ height: `${value}%` }} className={index === 6 ? 'active' : ''}/>)}</div><div className="growth-dates"><span>시작</span><span>첫 방문</span><span>체험</span><span>기록</span><span>성장</span><span>오늘</span></div><button onClick={() => setDetail('growth')}><BarChart3 /> 전체 성장 기록 보기</button></section></div>
...
function DetailPanel({ kind, progress, keywords, onClose }: { kind: 'zones' | 'keywords' | 'records' | 'growth'; progress: ReturnType<typeof buildProfileProgress>; keywords: Array<{ label: string; value: number; tone: string; icon: React.ReactNode }>; onClose: () => void }) {
  const titles = { zones: '나의 세종 탐험 지도', keywords: '키워드가 자라는 방법', records: '모든 탐험 기록', growth: '프로필 성장 기록' };
  return <div className="profile-detail-backdrop" onClick={onClose}><section className="profile-detail-panel" onClick={event => event.stopPropagation()}><header><div><small>MY SEJONG JOURNEY</small><h2>{titles[kind]}</h2></div><button onClick={onClose}><X /></button></header>
    ...
    {kind === 'records' && <div className="record-detail-list">{progress.records.length ? progress.records.map(item => <article key={item.id}><img src={item.image} alt=""/><div><small>{item.zone} · {formatTime(item.at)}</small><b>{item.title}</b><p>{item.note}</p></div><strong>+{item.point}P</strong></article>) : <div className="profile-empty"><BookOpen /><b>첫 기록을 기다리고 있어요</b><p>새로운 맵을 방문하면 자동으로 기록됩니다.</p></div>}</div>}
    {kind === 'growth' && <div className="growth-detail"><div className="growth-ring" style={{ '--score': `${progress.completion * 3.6}deg` } as CSSProperties}><b>{progress.completion}%</b></div><h3>현재까지 {progress.visitedZoneCount}개 권역, {progress.records.length}개의 순간을 기록했어요.</h3><p>새로운 권역 방문은 완성도와 탐험 점수를, 식물·생태·공연·교류 체험은 관심사와 기록 점수를 높여줍니다.</p><div><span>공간 탐험 <b>{progress.visitedZoneCount}/7</b></span><span>체험 데이터 <b>{progress.experienceCount}개</b></span><span>성장 포인트 <b>{progress.points}P</b></span></div></div>}
  </section></div>;
}

function keywordHint(label: string) {
  if (label.includes('자연')) return '호수공원·수목원·베어트리파크 방문과 식물 관찰로 성장해요.';
  if (label.includes('문화')) return '예술의전당·축제 체험과 공연 선택으로 성장해요.';
