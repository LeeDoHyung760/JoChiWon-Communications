...
import type { PortalPosition,WorldInteractionPosition } from '../../shared/socket-events';
...
export const GameCanvas=memo(function GameCanvas({profile,fullAccess}:{profile:UserProfile;fullAccess:boolean}){
  ...
  useEffect(()=>{
    ...
    let cancelled=false;
    const townRenderer=new VillageMapRenderer(ref.current,profile,LAKE_PARK_RENDERER_OPTIONS),bearTreeParkRenderer=new VillageMapRenderer(ref.current,profile,BEAR_TREE_PARK_RENDERER_OPTIONS),bearPlayZoneRenderer=new VillageMapRenderer(ref.current,profile,BEAR_PLAY_ZONE_RENDERER_OPTIONS);
    bearTreeParkRenderer.setVisible(false);bearPlayZoneRenderer.setVisible(false);
    const worldRenderers={town:townRenderer,'bear-tree-park':bearTreeParkRenderer,'bear-play-zone':bearPlayZoneRenderer};
    const savedPortalPositions=():PortalPosition[]=>[{destination:'bear-tree-park' as const,key:'world-portal-position-bear-tree-park'},{destination:'town' as const,key:'world-portal-position-town'}].flatMap(({destination,key})=>{try{const saved=JSON.parse(localStorage.getItem(key)??'null') as {x?:number;z?:number}|null;return saved&&Number.isFinite(saved.x)&&Number.isFinite(saved.z)?[{destination,x:saved.x!,z:saved.z!}]:[]}catch{return[]}});
    ...
    void Promise.all(Object.values(worldRenderers).map(renderer=>renderer.ready)).then(()=>{if(!cancelled)setLoading(false)}).catch(error=>{if(!cancelled)setLoadError(error instanceof Error?error.message:String(error))});
    ...
    game.scene.add('world',WorldScene,true,{profile,worldRenderers,fullAccess});
    return()=>{
      cancelled=true;
