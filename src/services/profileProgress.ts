...
export function buildProfileProgress(profile: UserProfile) {
  ...
  const scores = {
    nature: Math.min(100, 18 + zones.filter(z => ['town', 'bear-tree-park', 'bear-play-zone', 'garden', 'jochwon-park'].includes(z.id) && z.visited).length * 9 + greenhouse.collected.length * 4),
    culture: Math.min(100, 15 + zones.filter(z => ['arts-center', 'festival-experience', 'food-experience'].includes(z.id) && z.visited).length * 12 + countArray(lake?.activities) * 8),
    relation: Math.min(100, 12 + zones.filter(z => ['campus', 'student-hall', 'project-room'].includes(z.id) && z.visited).length * 10 + campus.length * 8 + friends.length * 4),
