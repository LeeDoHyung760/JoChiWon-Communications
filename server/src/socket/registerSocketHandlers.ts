...
import { normalizeLegacyMapId } from '../../../shared/map-ids.js';
import {recordAuthenticatedPlaceVisit} from '../services/profile/placeBehaviorService.js';
...
export function registerSocketHandlers(io:IO,socket:Client){
 ...
 const activeBearExplorers=()=>roomStore.playersIn('bear-tree-park').map(player=>player.id);
 ...
 socket.on('getPlayerResumeState',async ack=>{
  ...
  return ack(position?.mapId&&Number.isFinite(position.x)&&Number.isFinite(position.z)&&Number.isFinite(position.yaw)
   ?{mapId:normalizeLegacyMapId(position.mapId),x:position.x!,z:position.z!,yaw:position.yaw!,savedAt:position.savedAt?.getTime()??0}
   :null);
 });
 const enterMap=(payload:JoinMapPayload)=>{const previous=roomStore.players.get(socket.id);if(previous){socket.leave(previous.mapId);socket.to(previous.mapId).emit('userLeft',socket.id);publish(previous.mapId)}const matchProfile=payload.matchProfile?safeMatchProfile(payload.matchProfile):previous?.matchProfile;const player:PlayerState={id:socket.id,mapId:payload.mapId,x:payload.x,y:payload.y,direction:'down',isMoving:false,yaw:0,motionState:'idle',timestamp:Date.now(),nickname:clean(payload.nickname,16)||'조치원 주민',appearance:payload.appearance,model:payload.model,matchProfile};roomStore.players.set(socket.id,player);roomStore.recordDailyVisitor(socket.id);const authenticatedUserId=socket.data.userId as string|undefined;if(authenticatedUserId&&previous?.mapId!==payload.mapId)void recordAuthenticatedPlaceVisit(authenticatedUserId,payload.mapId).catch(error=>console.error('[place visit save failed]',error instanceof Error?error.name:'unknown'));socket.join(payload.mapId);const users=roomStore.playersIn(payload.mapId);socket.emit('worldClock',Date.now());socket.emit('portalPositionsUpdated',roomStore.allPortalPositions());socket.emit('bearTreePortalPositionsUpdated',roomStore.bearTreePortalPositions);socket.emit('interactionPositionsUpdated',roomStore.allInteractionPositions());socket.emit('lakeExperiencePositionsUpdated',roomStore.allLakeExperiencePositions());socket.emit('campusFeaturePortalPositionsUpdated',roomStore.allCampusFeaturePortalPositions());socket.emit('lakeWishesUpdated',roomStore.lakeWishes);socket.emit('nearbyChatHistory',roomStore.recentNearbyChat(payload.mapId));if(previous?.mapId==='bear-tree-park'||payload.mapId==='bear-tree-park')publishBearExploration(activeBearExplorers());io.emit('lakeDailyStatsUpdated',roomStore.lakeDailyStats());socket.emit('currentMapUsers',users);socket.to(payload.mapId).emit('userJoined',player);publish(payload.mapId)};
 ...
 socket.on('directChatRequest',async(toId,ack)=>{const from=roomStore.players.get(socket.id),to=roomStore.players.get(toId),fail=(message:string)=>{ack?.({ok:false,message});socket.emit('errorMessage',message)};if(!from||!to||from.id===to.id)return fail('접속 중인 사용자에게만 요청할 수 있어요.');if([...roomStore.pendingDirect.values()].some(request=>(request.fromId===from.id&&request.toId===to.id)||(request.fromId===to.id&&request.toId===from.id)))return fail('이미 처리 중인 채팅 요청이 있어요.');if([...roomStore.directRooms.values()].some(room=>room.active&&room.participants.some(p=>p.id===from.id)&&room.participants.some(p=>p.id===to.id)))return fail('이미 활성화된 1:1 채팅이 있어요.');if(!await directChatAllowed(socket.id,toId))return fail(AGE_GROUP_CHAT_RESTRICTED_MESSAGE);const requestId=crypto.randomUUID(),request={requestId,from:{id:from.id,nickname:from.nickname,appearance:from.appearance},toId};roomStore.pendingDirect.set(requestId,{fromId:from.id,toId});ack?.({ok:true,request});io.to(toId).emit('directChatRequested',request)});
 socket.on('directChatAccept',async requestId=>{const req=roomStore.pendingDirect.get(requestId);if(!req||req.toId!==socket.id)return;roomStore.pendingDirect.delete(requestId);if(!await directChatAllowed(req.fromId,req.toId)){io.to(req.fromId).emit('errorMessage',AGE_GROUP_CHAT_RESTRICTED_MESSAGE);return socket.emit('errorMessage',AGE_GROUP_CHAT_RESTRICTED_MESSAGE)}const first=roomStore.players.get(req.fromId),second=roomStore.players.get(req.toId),firstUserId=socketUserId(req.fromId),secondUserId=socketUserId(req.toId);if(!first||!second||!firstUserId||!secondUserId)return;const room=roomStore.createDirectRoom(first,second);try{await DirectRoomModel.findOneAndUpdate({roomId:room.id},{$set:{memberUserIds:[firstUserId,secondUserId],active:true}},{upsert:true,returnDocument:'after',runValidators:true})}catch(error){console.error('[direct room save failed]',error instanceof Error?error.name:'unknown');io.to(req.fromId).emit('errorMessage','채팅방을 저장하지 못했습니다.');return socket.emit('errorMessage','채팅방을 저장하지 못했습니다.')}room.participants.forEach(participant=>{const target=io.sockets.sockets.get(participant.id),partner=room.participants.find(candidate=>candidate.id!==participant.id);if(!target||!partner)return;target.join(room.id);io.to(participant.id).emit('directChatStarted',{...room,conversationId:room.id,partner})});});
 ...
 socket.on('directChatCancel',requestId=>{const req=roomStore.pendingDirect.get(requestId);if(!req||req.fromId!==socket.id)return;roomStore.pendingDirect.delete(requestId);io.to(req.toId).emit('directChatCancelled',{requestId,byId:socket.id});socket.emit('directChatCancelled',{requestId,byId:socket.id})});
 socket.on('directMessage',async({directRoomId,message:raw})=>{const player=roomStore.players.get(socket.id),room=roomStore.directRooms.get(directRoomId),message=clean(raw,500),userId=socket.data.userId as string|undefined;if(!player||!room?.active||!room.participants.some(participant=>participant.id===socket.id)||!message||!userId)return;const policy=await loadDirectChatPolicy(directRoomId,userId);if(!policy.allowed)return socket.emit('errorMessage',policy.code==='AGE_GROUP_CHAT_RESTRICTED'?AGE_GROUP_CHAT_RESTRICTED_MESSAGE:'채팅방 권한을 확인할 수 없습니다.');const messageId=crypto.randomUUID(),createdAt=Date.now();try{await DirectMessageModel.create({messageId,roomId:directRoomId,senderUserId:userId,message,type:'user',sentAt:new Date(createdAt)})}catch(error){console.error('[direct message save failed]',error instanceof Error?error.name:'unknown');return socket.emit('errorMessage','메시지를 저장하지 못했습니다.')}const data=roomStore.addDirectMessage({id:messageId,directRoomId,senderId:socket.id,nickname:player.nickname,message,createdAt,type:'user'});try{conversationInterestCache.merge(directRoomId,userId,localInterestKeywordExtractor.extract(message))}catch(error){console.warn('[conversation interest extraction failed]',error instanceof Error?error.name:'unknown')}io.to(directRoomId).emit('directMessageReceived',data)});
 socket.on('directChatClosed',directRoomId=>{const room=roomStore.directRooms.get(directRoomId);if(room?.active&&room.participants.some(p=>p.id===socket.id)){room.active=false;io.to(directRoomId).emit('directChatClosed',{directRoomId,byId:socket.id})}});
 ...
 socket.on('disconnect',()=>{const player=roomStore.players.get(socket.id);if(!player)return;void savePlayerPosition(player);for(const room of roomStore.directRooms.values())if(room.active&&room.participants.some(participant=>participant.id===socket.id)){room.active=false;io.to(room.id).emit('directChatClosed',{directRoomId:room.id,byId:socket.id})}roomStore.removePlayer(socket.id);socket.to(player.mapId).emit('userLeft',socket.id);publish(player.mapId);if(player.mapId==='bear-tree-park')publishBearExploration(activeBearExplorers())});
}
