...
export function registerSocketHandlers(io:IO,socket:Client){
 ...
 socket.on('saveRespawnPosition',async(position,ack)=>{
  const player=roomStore.players.get(socket.id);
  if(player?.mapId!=='town'||!roomStore.setRespawnPosition(position)){
   if(typeof ack==='function')ack({ok:false,position:roomStore.respawnPosition});
   return;
  }
  try{
   await saveWorldRespawnPosition(roomStore.respawnPosition);
   io.emit('respawnPositionUpdated',roomStore.respawnPosition);
   if(typeof ack==='function')ack({ok:true,position:roomStore.respawnPosition});
  }catch(error){
   console.error('[respawn position save failed]',error);
   if(typeof ack==='function')ack({ok:false,position:roomStore.respawnPosition});
  }
 });
