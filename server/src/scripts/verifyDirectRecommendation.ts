...
const connect=(nickname:string)=>new Promise<Client>((resolve,reject)=>{const socket:Client=io(base,{transports:['websocket']});socket.once('connect',()=>{socket.emit('joinMap',{mapId:'jochwon-station',nickname,appearance:{hair:'short',face:'smile',top:'green',bottom:'navy'},x:100,y:100,matchProfile:{mbti:'',interests:['카페','디저트'],usagePurposes:['대화'],preferredPlaceCategories:['카페']}});resolve(socket)});socket.once('connect_error',reject)});
