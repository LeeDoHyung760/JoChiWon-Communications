...
import { resolveConversationIntent,PLACE_INTENT_RULES } from '../services/places/placeIntentRules.js';
...
directRecommendationsRouter.post('/:directRoomId/recommendations',recommendationRateLimit,async(req,res)=>{
  ...
  inFlight.add(directRoomId);lastRequestedAt.set(directRoomId,now);io.to(directRoomId).emit('directRecommendationStarted',{directRoomId,stage:'analyzing'});
  try{
    ...
    const rawAnalysis=await analyzeConversation(participants,recentMessages,directRoomId,zoneName,rawRequest||undefined),analysis=resolveConversationIntent(rawAnalysis,recentMessages,env.DEFAULT_SEARCH_REGION);
    ...
    const search=await searchPlacesForIntent(analysis),places=scorePlaces(search.places,analysis);
