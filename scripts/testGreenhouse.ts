...
import { createFallbackMemoryLetter,dominantEmotion,GreenhouseProgressService,greenhouseCompletion,greenhouseInputLocked,normalizeMemoryText,parseGreenhouseProgress,type GreenhouseProgress } from '../src/services/greenhouseProgress';
...
let unlockProgress:GreenhouseProgress={collected:[],memoryLeaves:[],introSeen:true};
greenhousePlants.slice(0,13).forEach(plant=>{unlockProgress=service.collect(unlockProgress,plant.id,'희망','테스트')});
assert.equal(greenhouseCompletion(unlockProgress).unlocked,false,'13개에서는 기억나무가 잠겨야 한다');
unlockProgress=service.collect(unlockProgress,greenhousePlants[13].id,'평온','테스트');
assert.equal(greenhouseCompletion(unlockProgress).count,GREENHOUSE_PLANT_TOTAL);
assert.equal(greenhouseCompletion(unlockProgress).unlocked,true,'14개에서는 기억나무가 해금되어야 한다');
...
assert.deepEqual(parseGreenhouseProgress('{broken json'),{collected:[],memoryLeaves:[],introSeen:false},'깨진 저장 데이터는 안전하게 초기화해야 한다');
...
console.log('Greenhouse tests passed: collection, deduplication, emotion update, progress, unlock, fallback, recovery, input lock, mapping');
