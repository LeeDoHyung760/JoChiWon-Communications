...
- 충녕이의 전역 자율 순찰 경로와 서버 시간 기반 다중 사용자 위치 동기화 구현
- 세종호수공원 전체 지도 전용 사선 카메라, 고정 시야 및 지도·포탈 위치 저장 기능 구현
- 베어트리파크 GLB 월드, 3초 충전형 포탈 이동, 서버 공용 포탈·상호작용 좌표 동기화 구현
- 곰 놀이 공간 GLB 월드와 집 입구 근접 상호작용, bear cub 3D 모델·애니메이션 배치 구현
- 3D 모델 중복 로딩 제거, 텍스처 필터링, 적응형 렌더 해상도, 충돌·지면 계산 최적화를 통한 성능 개선
- 체험 모드의 전체 화면 입장 연출과 실시간 접속자·주변 채팅 UI 개선
- 여성형·남성형·커스텀형 캐릭터 선택과 머리·피부·상의·하의·신발·액세서리 색상 설정 및 월드 캐릭터 연동
- 수목원 GLB 맵 교체, 카메라·스폰 위치·충돌·식물 렌더링 및 입장 직후 프레임 저하 개선
- 수목원 12종 꽃과 복숭아나무·단풍나무의 관찰 포인트, 사진, 감정 기록형 AI 식물도감 UI 구현
- 실제 수집 식물과 감정만 사용하는 AI 기억 편지 검증, 입력 문장 정리 및 기억나무 편지지·식물 사진 UI 구현
- 세종시 2026년 공식 축제·공연 정보를 통합한 API, 데이터 모델, 이미지 콘텐츠 및 화면 연동 구현
- 세종 지역 음식점·전통시장 추천에 실제 현장 이미지와 출처 자료를 적용해 지역 콘텐츠 신뢰도 개선
- 세종호수공원 첫 입장 튜토리얼, 체험 진행 안내, 사용자 성향 기반 체험 추천 프로필 구현
- 충녕이 노트 UI와 호수공원 체험 기록·추천 흐름을 연결해 체험 콘텐츠 접근성 개선
- 맵별 캐릭터 재접속 위치 저장·복원과 포탈·상호작용 좌표 동기화 안정화
- 베어트리파크 숲 산책, AI 곰 생태 탐험, 수목원 식물 감정 기록을 하나의 자연 감성 발견 여정으로 연결
- 곰 놀이 공간 진입을 버튼 클릭 방식에서 3초 머무름 이동 방식으로 개선하고 곰 캐릭터를 정적인 관찰 대상으로 배치
- 곰 놀이 공간에 분리된 74개 파트의 공원 GLB 맵을 적용하고 스폰·카메라·지면 높이·충돌 처리를 새 지형에 맞게 조정
- 발자국·먹이·겨울 보금자리의 빛나는 흔적을 직접 찾는 탐험 UI와 반달가슴곰 생태 퀴즈·전문가 레벨 판정 구현
- 조사한 흔적을 바탕으로 자유 질문을 받을 수 있는 AI 생태 해설사와 서버 기반 안전 응답·대체 답변 흐름 구현
- 수목원 방문자의 식물 감정 기록을 익명으로 공유하고 기억나무에서 함께 살펴보는 공개 기억 기능 구현
- 곰 놀이 공간을 `베어트리 AI 탐험 연구소`로 재정의하고 폭포·동굴·큰 나무를 자유롭게 관찰하는 단일 생태 탐험 흐름 설계
- 방문 순서, 장소별 선택, 관람 속도, 사진 행동과 동행 방식을 종합해 여행 스타일을 분석하는 탐험 프로필 구현
- 폭포·동굴·큰 나무의 생태 설명과 행동 선택을 연결하고 완료한 관찰 지점만 반영하는 진행 상태 UI 구현
- AI가 사용자의 탐험 행동을 해석해 이동 방식·관람 속도·활동 선호·정보 선호·동행 방식을 제시하는 결과 화면 구현
- 실제 반달곰·불곰 발자국 사진 자료를 적용하고 발자국 특징과 판단 근거를 비교하는 곰 흔적 연구 콘텐츠 개선
- 베어트리파크와 세종호수공원의 포탈·포토존·체험 지점 좌표를 공용 서버 기준으로 고정하고 기존 위치 덮어쓰기 문제 해결
- 베어트리파크 입장 안내, 좌측 여행 스타일 진행 패널, 연구소 전용 프로필 표시 조건과 맵 전환 UI 개선
...
- 조치원 온실(Greenhouse) 식물 관찰·감정 기록·기억 나무 AI 체험 콘텐츠 구현
