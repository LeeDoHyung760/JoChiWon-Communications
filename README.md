...
- 세종호수공원 전체 지도 전용 사선 카메라와 고정 시야 구현
- 베어트리파크 GLB 월드와 3초 충전형 포탈 이동 구현
- 베어트리 AI 탐험 연구소 GLB 월드와 bear cub 3D 모델 배치 구현
...
- 수목원 GLB 맵과 카메라·스폰 위치·충돌·식물 관찰 지점 구현
- 식물 관찰, 사진, 감정 기록을 연결한 AI 식물도감 UI 구현
- 수집한 식물과 감정을 바탕으로 생성하는 AI 기억 편지와 기억나무 UI 구현
...
- 맵별 캐릭터 재접속 위치 저장·복원과 맵 전환 안정화
- 베어트리파크 숲 산책, AI 곰 생태 탐험, 수목원 식물 감정 기록을 하나의 자연 감성 발견 여정으로 연결
- 베어트리 AI 탐험 연구소의 폭포·동굴·큰 나무 관찰 지점과 자유 탐험 흐름 구현
- 방문 순서, 체류 시간, 장소별 선택, 포토 기록과 동행 선택을 저장하는 여행 행동 분석 구현
- 탐험 기록을 이동 방식·관람 속도·활동·정보·동행 성향으로 정리하는 여행 프로필 결과 화면 구현
- AI 응답 실패 시에도 기본 분석 결과를 제공하는 탐험 프로필 대체 처리 구현
- 베어트리파크 포탈·포토존·연구소 입구와 연구소 관찰 지점을 코드의 고정 좌표로 통일
- 베어트리파크 입장 안내, 좌측 여행 스타일 진행 패널과 연구소 전용 프로필 표시 조건 구현
