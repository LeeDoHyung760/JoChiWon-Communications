export type MapId = 'personal-farm' | 'town' | 'arts-center' | 'festival-experience' | 'food-experience' | 'club-street-festival' | 'bear-tree-park' | 'garden' | 'campus' | 'student-hall' | 'recruitment-center' | 'project-room' | 'government' | 'government-central-plaza' | 'government-policy-hall' | 'government-observatory' | 'sejong-smart-city' | 'jochwon-station' | 'traditional-market' | 'jochwon-park' | 'college-street';
...
export interface DirectChatStartedPayload extends DirectRoom { conversationId:string; partner:Pick<PlayerState,'id'|'nickname'|'appearance'|'matchProfile'> }
...
export interface ServerToClientEvents {
 ...
 nearbyChat:(message:ChatMessage)=>void; nearbyChatHistory:(messages:ChatMessage[])=>void; directChatRequested:(request:DirectRequest)=>void; directChatRejected:(data:{requestId:string;byId:string})=>void; directChatCancelled:(data:{requestId:string;byId:string})=>void; directChatStarted:(room:DirectChatStartedPayload)=>void; directMessageReceived:(message:DirectMessage)=>void; directChatClosed:(data:{directRoomId:string;byId:string})=>void;
...
export interface ClientToServerEvents {
 ...
 directChatRequest:(toId:string,ack?:(result:{ok:boolean;request?:DirectRequest;message?:string})=>void)=>void; directChatAccept:(requestId:string)=>void; directChatReject:(requestId:string)=>void; directChatCancel:(requestId:string)=>void; directMessage:(data:{directRoomId:string;message:string})=>void; directChatClosed:(directRoomId:string)=>void;
